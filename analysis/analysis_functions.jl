...
function analyze_afm(;folder = "")
    ...
    limit = 0.05*maximum(force)

    diffs = diff(force)

    negs = findall(diffs .< 0 .&& abs.(diffs) .> limit)
    if length(negs) > 0
        force = force[1:negs[1]]
        depth = depth[1:negs[1]]
    end
...
function analyze_forces(forceName; folder = "", output = "all")
    ...
    if !in(forceName,allowedForces)
        println("Force called \""*forceName*"\" not found.")
        return
    end
    ...
    temp = zeros(Bool,length(allFiles))
    ...
    enveFiles = allFiles[findall(temp)]
    ...
    if output == "all"
        ...
        for i = 1:length(means)
            ...
            forceData =  get_data(pointData[forceName])

            forceValues = zeros(size(forceData)[2])

            for j = 1:size(data)[2]
                forceValues[j] = norm(forceData[:,j])
            end

            means[i] = mean(forceValues)
            stds[i] = std(forceValues)
        end
    elseif output == "first"
        ...
        for j = 1:size(forceData)[2]
            forceValues[j] = norm(forceData[:,j])
        end

        means = mean(forceValues)
        stds = std(forceValues)
    elseif output == "last"
        vtk = VTKFile(folder*"/"*enveFiles[end])
